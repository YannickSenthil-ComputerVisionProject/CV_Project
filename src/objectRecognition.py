...
class ExtractTiles():
    ...
    def plot_points(self,image,points,title,show_line = False):
        ...
        if self.save == True:
            cv2.imwrite('Outputs_From_Code/line.jpg',image)
        self.show_image(image,title)

    def meddistance(self,points):
        dist = []
        for i in xrange(len(points)-1):
            dist.append(self.distance_function(points[i,0],points[i+1,0],points[i,1],points[i+1,1]))
    ...
    def get_contours(self,show = False):
        ...
        if self.save == True:
            cv2.imwrite('Outputs_From_Code/cavitycontours.jpg',objects)
        ...
        for i in xrange(length):
            for j in xrange(length):
                if j>i:
                    ...
                    if counter >= 5:
                        #points = np.array(points)[np.array(points)[0,:].argsort()]
                        #dev = self.meddistance(points)
    ...
    def crop_image(self,show = False):
        ...
        if self.save == True:
            cv2.imwrite('Outputs_From_Code/cropped.jpg',self.crop)
        if show == True  or self.show == True:
    ...
    def get_tile_contour(self,show = False):
        ...
        self.show_image(adapt,"adaptive threshholding")
        ...
        if detected_contours == 1:
            print "laaaaaaaaaaaaaaaaaaaaaaaaaaa", center_points
            self.tile_center_point= center_points
            self.tile_center_point= self.tile_center_point.flatten()
        else:
            print "olaaaaaaaaaaaaaaaaaaaaaaaaaaa", center_points
        ...
        if self.save == True:
            cv2.imwrite('Outputs_From_Code/5tiles.jpg',self.objects)
        if show == True  or self.show == True:
    ...
    def find_corners(self,show = False):
        ...
        dst = cv2.cornerHarris(gray,7,21,0.07)
        ...
        if self.save == True:
            cv2.imwrite('Outputs_From_Code/Corners.jpg',self.objects[0:50,0:50])
        if show == True  or self.show == True:
    ...
    def sort_points(self):
        ...
        self.corners = self.final_corner_points
        ...
        self.final_corner_points = self.corners
    ...
    def homography(self,show = False):
        ...
        if self.save == True:
            cv2.imwrite('Outputs_From_Code/homography.jpg',self.dst)
        if show == True  or self.show == True:
    ...
    def get_tiles(self,show = False):
        ...
        for i in xrange(5):
            ...
            cv2.imwrite('Outputs_From_Code/tile'+str(i)+'.jpg',self.dst[0:200,200*i:200*(i+1)])
...
class Find_objects():
    ...
    def compare_this_image_with_database(self,img):
        images = [cv2.imread(file) for file in glob.glob("DataBase/*.jpg")]

        Image_names = glob.glob("DataBase/*.jpg")

        for i in range (len(Image_names)):
            Image_names[i]=Image_names[i].replace("DataBase/","")
